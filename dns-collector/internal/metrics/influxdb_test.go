...
func TestInfluxDBClientPushMetricsPartialFailure(t *testing.T) {
	...
	// We can't easily test the actual HTTP calls without a real InfluxDB,
...
func TestInfluxDBClientMetricConversion(t *testing.T) {
	...
	registry.ServerNewDomains.Inc()
	registry.ServerNewDomains.Add(5)
	registry.DBDomainsTotal.Set(1000)
	registry.ResolverActiveWorkers.Set(10)
	registry.ResolverLookupDuration.WithLabelValues("ipv4").Observe(0.05)
	registry.ResolverLookupDuration.WithLabelValues("ipv6").Observe(0.1)
...
func TestInfluxDBClientContextTimeout(t *testing.T) {
	// Test that client respects context timeouts
	cfg := config.InfluxDBConfig{
